...
import staicoin.server.ws_connection as ws  # lgtm [py/import-and-import-from]
...
from staicoin.consensus.pot_iterations import calculate_sp_interval_iters
from staicoin.farmer.pooling.constants import constants
from staicoin.farmer.pooling.og_pool_state import OgPoolState
from staicoin.farmer.pooling.pool_api_client import PoolApiClient
from staicoin.pools.pool_config import PoolWalletConfig, load_pool_config
from staicoin.protocols import farmer_protocol, harvester_protocol
from staicoin.protocols.pool_protocol import (
    ErrorResponse,
...
from staicoin.server.outbound_message import NodeType, make_msg
from staicoin.server.server import ssl_context_for_root
from staicoin.server.ws_connection import WSstaicoinConnection
from staicoin.ssl.create_ssl import get_mozilla_ca_crt
from staicoin.types.blockchain_format.proof_of_space import ProofOfSpace
from staicoin.types.blockchain_format.sized_bytes import bytes32
from staicoin.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from staicoin.util.byte_types import hexstr_to_bytes
from staicoin.util.config import load_config, save_config, config_path_for_filename
from staicoin.util.hash import std_hash
from staicoin.util.ints import uint8, uint16, uint32, uint64
from staicoin.util.keychain import Keychain
from staicoin.wallet.derive_keys import (
...
from staicoin.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    def __init__(
        ...
        if len(self.get_public_keys()) == 0:
            error_str = "No keys exist. Please run 'staicoin keys generate' or open the UI."
        ...
        self.pool_target_encoded = pool_config["stai_target_address"]
    ...
    async def on_connect(self, peer: WSstaicoinConnection):
    ...
    def on_disconnect(self, connection: ws.WSstaicoinConnection):
    ...
    async def get_cached_harvesters(self, connection: WSstaicoinConnection) -> HarvesterCacheEntry:
