...
from stai.pools.pool_config import PoolWalletConfig, load_pool_config, add_auth_key
...
from stai.util.config import load_config, save_config, config_path_for_filename, get_config_lock
            ...
            while not self._shut_down:
                if await self.setup_keys():
                    ...
                    if not self.is_pooling_enabled():
                        self.log.info(
                            f"Not OG pooling as 'pool_payout_address' and/or 'pool_url' are missing in your config")
                    ...
                    self.adjust_pool_difficulties_task = asyncio.create_task(
                        self._periodically_adjust_pool_difficulties_task())
